...
//         final token = sl<GetStorageData>().getToken()?.trim();

//         if (token != null && token.isNotEmpty) {
//           // Refresh token automatically if expired
//           if (Jwt.isExpired(token)) {
//             final refreshed = await _handleExpiredToken();
//             if (!refreshed) {
...
//         // Retry on 401 automatically once
...
//   final token = sl<GetStorageData>().getToken()?.trim();
//   final refresh = sl<GetStorageData>().getRefreshToken();

//   print("Current accessToken: $token");
//   print("Current refreshToken: $refresh");

//   if (token == null || token.isEmpty) return false;

//   if (!Jwt.isExpired(token)) return true;
...
//   if (!refreshed) {
//     print("Refresh failed, deleting tokens");
//     sl<GetStorageData>().deleteToken();
//     return false;
//   }
...
// static Future<bool> _handleExpiredToken() async {
//   final refreshToken = sl<GetStorageData>().getRefreshToken();

//   if (refreshToken == null || refreshToken.isEmpty) return false;

//   try {
//     final response = await sl<Remote>()
//         .refreshToken(PostRefreshToken()..refreshToken = refreshToken);
...
class DioHelper {
 ...
 Future<bool> checkLogin(bool isLogin) async {
  try {
    ...
    // لو مفيش token خالص
    if (currentToken == null || currentToken.isEmpty) {
      sPrint.warning('⚠️ No access token found');
      final refreshed = await _handleExpiredToken();
print("refreshed tokennnnnn=>$refreshed");
    return refreshed;
    }
    ...
    // ✅ Token expired → استدعي الـ refresh (هيستخدم Dio مباشرة)
    sPrint.info('Token expired, attempting refresh...');
    final refreshed = await _handleExpiredToken();

    return refreshed;
    
  } catch (e, s) {
    sPrint.error('checkLogin error: $e', s);
    return false;
  }
}
...
static Future<bool> _handleExpiredToken() async {
  ...
  try {
    ...
    if (response.status == 1 && response.data != null) {
      ...
      print("new access tokeen=>${response.data!.accessToken!}");
      return true;
    }
